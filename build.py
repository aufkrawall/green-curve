...
def verify_release_binary(path, os_name, arch, allow_debug_paths=False, original_filename=None):
    ...
    if os_name == "windows":
        pe_verify.verify_windows_binary_metadata(
            data, os.path.basename(path), original_filename or "", arch,
            ACTIVE_WINDOWS_TOOLCHAIN)
...
def run_source_regression_checks():
    ...
    require_text(shared_h, "SERVICE_PROTOCOL_VERSION = 28",
                 "service protocol publishes outcome severity, update state, XBAR and the handback lockout")
