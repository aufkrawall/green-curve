...
SOURCE_SIZE_RATCHET = {
    ...
    "gpu_core.h": 829,  # 910 before validate_desired_settings_for_ipc moved to desired_settings_ipc.h  # 896 before F-01-002 gave the IPC boundary the fan curve's own validity rule  # 881 before the NVML utilisation/power-usage load witnesses  # 862 before POWER_LIMIT_DEFAULT_PCT and the IPC unknown-power sentinel  # 842 before the mem effective/display parity helpers
    ...
    "main.cpp": 796,  # 801 before the wedge-watchdog heartbeats moved to service_gate_progress.h  # 800 before F-03-001 made the log tokenizers visible to every shard
...
#
# The GUI and the service get SEPARATE resource scripts so each binary's
# VERSIONINFO names the file it actually ships as.  Sharing one script made
# greencurve-service.exe claim OriginalFilename "greencurve.exe", which is the
...
WINDOWS_BINARY_IDENTITIES = {
    ...
    True: ("Green Curve background service", "GreenCurveService", "greencurve-service.exe"),
}
# Only the GUI owns a notification-area icon (entry.cpp loads 111-115).  The
# service has no window, so its image carries just the file icon: five unused
# PNG-compressed icon groups would otherwise be a large high-entropy resource
# blob in a SYSTEM service.  pe_verify.verify_service_resources() gates it.
TRAY_ICON_RC_LINES = """111 ICON "greencurve_tray_default.ico"
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "InternalName", "GC_INTERNAL_NAME"
            VALUE "LegalCopyright", "Copyright (c) 2026 aufkrawall. MIT License."
            VALUE "OriginalFilename", "GC_ORIGINAL_FILENAME"
...
def compile_windows_resources(rc_tool, version, build_number, manifest_path,
    ...
    for rc_path, res_path, service in variants:
        ...
        if os.path.exists(res_path):
            os.remove(res_path)
        print(f"Compiling resources: {os.path.basename(rc_path)}")
        result = subprocess.run([rc_tool, "/x", f"/fo{res_path}", rc_path], cwd=cwd)
