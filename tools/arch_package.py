...
# Every key pacman's install-time .PKGINFO parser (libalpm be_package.c
# parse_descfile) accepts.  Anything else makes `pacman -U` log
# "unknown key '<key>' in package description".  `makepkgopt` left .PKGINFO
# with pacman 5.0 (2016) in favour of .BUILDINFO, and the install scriptlet is
# the package's `.INSTALL` member -- pacman never had an `install` key.
PKGINFO_KNOWN_KEYS = frozenset((
    "pkgname", "pkgbase", "pkgver", "pkgdesc", "url", "builddate", "packager",
    "size", "arch", "license", "replaces", "group", "provides", "conflict",
    "backup", "depend", "optdepend", "makedepend", "checkdepend", "xdata",
))


def validate_pkginfo_keys(text):
    """Reject any .PKGINFO key pacman would warn about during installation.

    Returns the parsed keys in file order so tests can assert on the emitted
    metadata.  Comment and blank lines are ignored; a key line without '=' is
    a malformed package and fails rather than being skipped silently.
    """
    keys = []
    for lineno, raw in enumerate(text.splitlines(), 1):
        ...
        if "=" not in line:
            raise RuntimeError(
                f".PKGINFO line {lineno} is not a key = value pair: {line!r}")
        key = line.split("=", 1)[0].strip()
        if key not in PKGINFO_KNOWN_KEYS:
            raise RuntimeError(
                f".PKGINFO key '{key}' (line {lineno}) is unknown to pacman's "
                "install-time parser and warns on every package install")
        keys.append(key)
    return keys
...
def build_arch_package(script_dir, app_version, arch, binary_path, pkgrel=1, output_dir=None):
    ...
    # Only keys from PKGINFO_KNOWN_KEYS: pacman logs
    # "unknown key '<key>' in package description" for anything else on every
    # `pacman -U`.  The .INSTALL scriptlet ships as a package member and needs
    # no .PKGINFO key; pacman discovers it by name.
    pkginfo_lines = [
        ...
        for member in tar.getmembers():
            ...
            if member.isdir():
                ...
                if member.name == ".PKGINFO":
                    ...
                    # Fails the build on any key pacman warns about at install
                    # time (`makepkgopt`/`install` regression).
                    validate_pkginfo_keys(text)
...
def run_self_tests():
    ...
    Pins the .PKGINFO key allowlist: the generated package metadata may only
    carry keys pacman's install-time parser knows, so `pacman -U` stays free
    of "unknown key '<key>' in package description" warnings.  The first
    generated packages shipped `makepkgopt` (a legacy key removed from
    .PKGINFO in pacman 5.0) and an `install` key that pacman never had; both
    warned on every install.
    """
    failures = []
    ...
    good = "\n".join([
        ...
        "pkgbase = greencurve",
        "pkgver = 1.0-1",
        "pkgdesc = NVIDIA VF curve daemon & terminal UI",
        "url = https://github.com/aufkrawall/green-curve",
        ...
        "license = MIT",
        "depend = glibc",
        "optdepend = nvidia-utils: proprietary NVIDIA driver libraries",
        "",
    ])
    try:
        keys = validate_pkginfo_keys(good)
    except RuntimeError as exc:
        failures.append(f"the generated .PKGINFO shape must validate: {exc}")
    else:
        if keys[:3] != ["pkgname", "pkgbase", "pkgver"] or len(keys) != 12:
            failures.append("validate_pkginfo_keys must preserve file order and count")

    try:
        repeated = validate_pkginfo_keys("# comment\n\nlicense = MIT\nlicense = GPL\n")
    except RuntimeError as exc:
        failures.append(f"comment/blank lines and repeatable keys must validate: {exc}")
    else:
        if repeated != ["license", "license"]:
            failures.append("comments must be skipped and repeatable keys kept in order")

    # The two keys that shipped in the first generated packages are exactly
    # the two pacman warns about on every install; each must be rejected.
    for bad_key, value in (("makepkgopt", "strip"), ("install", ".INSTALL")):
        try:
            validate_pkginfo_keys(f"pkgname = greencurve\n{bad_key} = {value}\n")
        except RuntimeError as exc:
            if bad_key not in str(exc):
                failures.append(f"rejection of '{bad_key}' must name the key: {exc}")
        else:
            failures.append(
                f"'{bad_key}' must be rejected: pacman warns 'unknown key' on install")

    try:
        validate_pkginfo_keys("pkgname = greencurve\npkgtypo = 1\n")
    except RuntimeError:
        pass
    else:
        failures.append("unknown keys must be rejected")
    ...
    try:
        ...
        failures.append("a line without '=' must fail rather than be skipped")

    if failures:
        for failure in failures:
            print(f"arch_package self-test FAILED: {failure}")
        sys.exit(1)
    print("arch_package self-tests passed")
