...
void merge_desired_settings(DesiredSettings* base, const DesiredSettings* incoming) {
    ...
    if (incoming->hasXbarOffsetKhz) {
        base->hasXbarOffsetKhz = true;
        base->xbarOffsetKhz = incoming->xbarOffsetKhz;
    }
    if (incoming->hasXbarMsvddOffsetUv) {
        base->hasXbarMsvddOffsetUv = true;
        base->xbarMsvddOffsetUv = incoming->xbarMsvddOffsetUv;
    }
    if (incoming->hasSysClkOffsetKhz) {
...
static bool load_desired_settings_from_sections(const IniDocument* doc,
    ...
    if (!value.empty()) {
        if (!parse_int_strict(value.c_str(), &desired->xbarOffsetKhz) ||
            desired->xbarOffsetKhz < -1000000 ||
            desired->xbarOffsetKhz > 1000000) {
            set_message(err, errSize,
                        "Invalid or out-of-range xbar_offset_khz in %s",
                        messageContext);
            return false;
        }
    ...
    value = get_section_value(doc, controlsSection, "xbar_msvdd_offset_uv");
    if (!value.empty()) {
        if (!parse_int_strict(value.c_str(), &desired->xbarMsvddOffsetUv) ||
            desired->xbarMsvddOffsetUv < -100000 ||
            desired->xbarMsvddOffsetUv > 100000) {
            set_message(err, errSize,
                        "Invalid or out-of-range xbar_msvdd_offset_uv in %s",
                        messageContext);
            return false;
        }
    ...
    value = get_section_value(doc, controlsSection, "sys_clk_offset_khz");
    if (!value.empty()) {
        if (!parse_int_strict(value.c_str(), &desired->sysClkOffsetKhz) ||
            desired->sysClkOffsetKhz < -1000000 ||
            desired->sysClkOffsetKhz > 1000000) {
            set_message(err, errSize,
                        "Invalid or out-of-range sys_clk_offset_khz in %s",
                        messageContext);
            return false;
        }
