...
// Stopping what is running lives in installer_stop.cpp, included here so the
// amalgamated ordering is unchanged: the helpers above (gc_set_error,
// gc_report, the failure log) and the settings-capture code below all see the
// same static state.  gc_stop_gui_processes() remains a normal function for
// the uninstall shard through installer_common.h.
#include "installer_stop.cpp"
